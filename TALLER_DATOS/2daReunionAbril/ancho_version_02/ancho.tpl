//Codigo Modelo de Evaluacion Anchoveta 2021 version proyeccion
...
DATA_SECTION
 ...
 init_vector uno(1,nrec)
 init_vector dos(1,nrec)
...
// !!cout << "Numero_Rec: " << nrec << endl;exit(1);
...
//Defino las variables de estado 
 ...
 3darray CTP(1,npbr,1,ntime_sim,1,nedades)
 ...
 3darray matF(1,npbr,1,ntime_sim,1,nedades)
 3darray Npy(1,npbr,1,ntime_sim,1,nedades)
 3darray PCpy(1,npbr,1,ntime_sim,1,nedades)   
 vector runo(1,nrec)
 vector rdos(1,nrec)
 vector Runo(1,NREC)
 vector Rdos(1,NREC)
...
FUNCTION Eval_CTP
  ...
  for(int j=1;j<=npbr;j++){
    Np=N(ntime);
    if(opt_Rcase==3 || opt_Rcase==4){Np(1)=Np(1)*0.3723793;Reclutas(ntime)=Np(1);}
    Sp=S(ntime);
    Fp=Ftot(ntime);
    if(opt_Rcase==1 || opt_Rcase==3){
      for(int r=1;r<=nrec;r++){runo(r)=Reclutas(uno(r));}
      for(int s=1;s<=nrec;s++){rdos(s)=Reclutas(dos(s));}
    } else if(opt_Rcase==2 || opt_Rcase==4){
      for(int r=1;r<=nrec-2;r++){Runo(r)=Reclutas(uno(r));}
      for(int s=1;s<=nrec-2;s++){Rdos(s)=Reclutas(dos(s));}
    }
    for(int i=1;i<=ntime_sim;i++){
      ...
      if(opt_Rcase==1 || opt_Rcase==3){
        if(i%2==0){Npy(j,i)(1)=pR*mean(rdos);};if(i%2!=0){Npy(j,i)(1)=pR*mean(runo);}
      } else if(opt_Rcase==2 || opt_Rcase==4){
        if(i%2==0){Npy(j,i)(1)=pR*mean(Rdos);};if(i%2!=0){Npy(j,i)(1)=pR*mean(Runo);}
      }
      if(i==1 && opt_Rcase==1){Fpbr=0.27*(Fp/max(Fp))*alfe;}
      else if(i==2 && opt_Rcase==1){Fpbr=0.39*(Fp/max(Fp))*alfe;}
      else if(i==1 && opt_Rcase==2){Fpbr=0.27*(Fp/max(Fp))*alfe;}
      else if(i==2 && opt_Rcase==2){Fpbr=0.39*(Fp/max(Fp))*alfe;}
      else if(i==1 && opt_Rcase==3){Fpbr=0.66*(Fp/max(Fp))*alfe;}
      else if(i==2 && opt_Rcase==3){Fpbr=0.48*(Fp/max(Fp))*alfe;}
      else if(i==1 && opt_Rcase==4){Fpbr=0.66*(Fp/max(Fp))*alfe;}
      else if(i==2 && opt_Rcase==4){Fpbr=0.52*(Fp/max(Fp))*alfe;}
      else {Fpbr=(Fp/max(Fp))*pbr(j);}
      matF(j,i)=Fpbr;
      Zpbr=Fpbr+M;
      Sp=mfexp(-1.0*Zpbr);
      NMDpy(j,i)=elem_prod(Npy(j,i),mfexp(-dt(4)*Zpbr))*Prob_talla;
      BDpy(j,i)=sum(elem_prod(elem_prod(NMDpy(j,i),msex),Wmed(ntime)));
      CTP(j,i)=elem_prod(elem_div(Fpbr,Zpbr),elem_prod(Npy(j,i),(1-Sp)));
// cout << "CTP" << CTP << endl;exit(1);
      PCpy(j,i)=CTP(j,i)/sum(CTP(j,i));
      YTP(j,i)=sum(elem_prod(CTP(j,i)*Prob_talla,Wmed(ntime)));
      RPRpy(j,i)=BDpy(j,i)/SSBo;
   }
 }
 for(int k=1;k<=npbr;k++){CBA(k)=YTP(k,sem1)+YTP(k,sem2);}
